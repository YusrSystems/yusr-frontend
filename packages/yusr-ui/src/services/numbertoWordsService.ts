...
export class NumbertoWordsService
{
  ...
  static ConvertAmount(amount: number, currency: Currency): string
  {
    ...
    if (integerPart > 0)
    {
      result += this.Convert(integerPart, currency.isFeminine) + " "
    ...
    if (fractionPart > 0)
    {
      ...
      result += this.Convert(fractionPart, currency.isFeminine) + " "
  ...
  public static Convert(num: number, isFeminine = false): string
  {
    ...
    if (num >= 1000)
    {
      ...
      {
        parts.push(this.convertHundreds(thousands, false) + " آلاف");
      }
      num %= 1000;
    }
