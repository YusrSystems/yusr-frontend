...
)
{
	...
	const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) =>
	{
		...
		update(multiple ? [...existing, ...newStorageFiles] : newStorageFiles[0] ? [newStorageFiles[0]] : []);
	...
	const handleRemoveFile = (index: number) =>
	{
		...
		const removeFile = (file: StorageFile | undefined): StorageFile | undefined =>
		{
	...
	{
		const files = [...(getValue() ?? [])];
		if (index === 0 || files[0] == undefined || files[index] == undefined)
		{
			return;
		}
