...
export default function ChangeAccountDialogOld({
  ...
  selectEntityState,
  selectFormState
}: CommonChangeDialogPropsOld<AccountOld> & {
...
})
{
  ...
  const initialValues = useMemo(
    () => ({
      ...
      ...entity,
      name: entity?.name || "",
      accountContacts: entity?.accountContacts || [new AccountContactOld()]
    }),
    [entity]
  );
  ...
  const addContact = () =>
  {
    dispatch(slice.formActions.updateFormData({
      accountContacts: [...(formData.accountContacts || []), new AccountContactOld()]
    }));
  };

  const updateContact = (
    index: number,
    field: keyof AccountContactOld,
    value: any
  ) =>
  {
