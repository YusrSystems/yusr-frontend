...
export function PartnerStatementReportTable()
{
	useSignals();

	if (Cubits.PartnerStatementReport.state.value instanceof ReportLoading)
	{
		return <TablePreview.Loading/>;
	}

	if (Cubits.PartnerStatementReport.state.value instanceof ReportLoaded)
	{
		const rows = Cubits.PartnerStatementReport.result.value?.lines ?? [];
		const partnerType = Cubits.PartnerStatementReport.result.value?.partner.type ?? PartnerType.Customer;
		const isDebitNormal = partnerType === PartnerType.Customer;
		...
		return (
			<table className="w-full mt-5 border-collapse rounded-lg overflow-hidden">
				...
				{ rows.map((row, idx) =>
				{
					...
					const favorableWhenPositive = isDebitNormal;
					...
					const debitClassName =
						row.debit > 0
							? favorableWhenPositive && isDebitNormal
								? greenColorClass
								: redColorClass
							: undefined;
					...
					return (
						<tr key={ `${ row.id }-${ idx }` }>
							...
							{ route ? (
								...
								<ReportTableTd isEven={ isEven }>
									{ row.documentId > 0 ? row.documentId : "-" }
								</ReportTableTd>
							) }
							...
							<ReportTableTd isEven={ isEven } className={ debitClassName }>
								{ formatNumber(row.debit) }
							</ReportTableTd>
							<ReportTableTd isEven={ isEven } className={ creditClassName }>
								{ formatNumber(row.credit) }
							</ReportTableTd>
							<ReportTableTd isEven={ isEven } className={ runningBalanceClassName }>
