...
export function StockValuationReportPage()
{
	...
	// Safely extract and type the data from the state without using generics on ReportLoaded
	const reportData = ("data" in state && state.data)
		? (state.data as StockValuationReportResult)
		: null;

	// Store the active request to be reused in pagination
	...
	useEffect(() =>
	{
		...
		void Cubits.stockValuationReport.getReportData(initialRequest, 1, 100);
	}, [lastRequest]);
