		...
		<section id="pricing" dir={ i18n.dir() } className="mx-auto max-w-6xl px-6 py-20">
			...
			<div className="overflow-hidden rounded-2xl bg-card border border-primary/20 shadow-xl shadow-primary/5">
				...
				<div className="grid grid-cols-[1fr_auto]">
					...
					{ /* Right: Price */ }
					<div
						className="border-b border-border bg-primary px-8 py-5 text-center text-primary-foreground w-48">
						<div className="flex items-baseline justify-center gap-1">
							{ billing === "yearly" &&
                                <span className="text-sm line-through opacity-60">{ monthlyPrice }</span> }
							<span className="text-4xl font-extrabold">{ price }</span>
						</div>
						<span
							className="text-sm opacity-80">{ t("pricing.plan.currency") }{ t("pricing.plan.perMonth") }</span>
				...
				<div className="grid grid-cols-[1fr_auto]">
					...
					<div
						className="bg-primary/8 dark:bg-primary/12 min-w-48 px-6 py-5 flex items-center justify-center">
						<Link to="/register">
